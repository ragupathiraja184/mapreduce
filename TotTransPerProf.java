...
public class TotTransPerProf {
	...
	public static class StoreMapper extends Mapper<LongWritable,Text,Text,Text>
	{
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
		{
			...
			String[] valueArr1 = value.toString().split(",");
			String custId = valueArr1[2];
			String orderPrice = valueArr1[3];
	...
	public static class MyReducer1 extends Reducer<Text,Text,Text,DoubleWritable>
	{
		public void reduce(Text key,Iterable<Text> value, Context context) throws IOException, InterruptedException
		{
			...
			for(Text val : value)
			{
				String valArr2[] = val.toString().split(",");
				String marker = valArr2[0];
				if(marker.equals("s"))
				{
					double orderPrice = Double.parseDouble(valArr2[1]);
					//occCount++;
					tot+=orderPrice;
				}
	...
	public static class Mapper2 extends Mapper<LongWritable,Text,Text,DoubleWritable>
	{
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
		{
			String[] valueArr3 = value.toString().split("\t");
			context.write(new Text(valueArr3[0]), new DoubleWritable(Double.parseDouble(valueArr3[1])));
		}
	}
	...
	public static class SortMapper extends Mapper<LongWritable,Text,DoubleWritable,Text>
	{
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException
		{
			String[] valueArr4 = value.toString().split("\t");
			context.write(new DoubleWritable(Double.parseDouble(valueArr4[0])), new Text(valueArr4[1]));
		}
	}
